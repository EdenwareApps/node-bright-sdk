...
class BrightDataSDK {
	constructor(consoleObj, dllPath){
		...
		if(this.ref){
			...
			if(this.ffi){
				this.console.log('[bright data sdk] FFI loaded.')
				const start = this.time(), ks = Object.keys(this.table)
				ks.forEach((k) => {
					this.bindings[this.table[k][0]] = this.table[k][1]
				})
				this.console.log('[bright data sdk] Loading DLL', this.dllPath)
				const lum = new this.ffi.Library(this.dllPath, this.bindings)
				this.loadTime = (this.time() - start)
				this.console.log('[bright data sdk] Lum library loaded in ' + this.loadTime + ' secs.')
				ks.forEach((k) => {
					this[k] = lum[this.table[k][0]]
				})
			} else {
				this.console.error('[bright data sdk] No ffi module available.')
			}
		} else {
			this.console.error('[bright data sdk] No ref module available.')
		}
	}
	time(){
...
module.exports = (console, dllPath) => {	
	if(process.platform.substr(0, 3) == 'win'){
		return new BrightDataSDK(console, dllPath)
	} else {
		let err = new Error('[bright data sdk] Windows only for now.')
		if(console){
			console.error(err)
		}
		return err
