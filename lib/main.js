...
class BrightSDK {
	constructor(consoleObj, dllPath){
		...
		if(this.ref){
			...
			if(this.ffi){
				this.console.log('[bright sdk] FFI loaded.')
				...
				this.console.log('[bright sdk] Lum library loaded in ' + this.loadTime + ' secs.')
				ks.forEach((k) => {
					this[k] = lum[this.table[k][0]]
				})
			} else {
				this.console.error('[bright sdk] No ffi module available.')
			}
		} else {
			this.console.error('[bright sdk] No ref module available.')
		}
	}
	time(){
...
module.exports = (console, dllPath) => {	
	if(process.platform.substr(0, 3) == 'win'){
		return new BrightSDK(console, dllPath)
	} else {
		let err = new Error('[bright sdk] Windows only for now.')
		if(console){
			console.error(err)
		}
		return err
