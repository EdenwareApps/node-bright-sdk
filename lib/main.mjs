...
export default class BrightSDK extends EventEmitter {
	...
	async prepare() {
		...
		for (const key in this.tableDef) {
			...
			this[key] = (...args) => {
				if (this.debug) console.log(`Calling ${key}`, args)
				return new Promise((resolve, reject) => {
					this.functions[key].async(...args, (err, val) => {
						if (this.debug) console.log(`Called ${key}`, { err, val })
		...
		const ChoiceChangeCallback = ret => {
			if (this.debug) console.log('Choice change callback', { ret })
			process.nextTick(() => {
				try {
		...
		await new Promise((resolve, reject) => {
			this.functions.setChoiceChangeCallback.async(this._choiceCbHandle, err => {
				if (err) {
					...
					if (this.debug) console.log('Choice change callback registered')
					resolve()
				}
			})
		})
